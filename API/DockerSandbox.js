 ...
 */
var DockerSandbox = function(timeout_value,path,folder,parameters,vm_name,compiler_name,file_name,code,output_command,languageName,e_arguments,stdin_data)
{
...
DockerSandbox.prototype.execute = function(success)
{
	...
	if(this.parameters!=null && this.parameters!="" && execSync('docker ps -a -q --no-trunc|grep -c ' + this.parameters + ' || true') >0){ //|| true pour que la commande n'échoue jamais
	    console.log("Resume : " + this.parameters);
	    var ip=execSync('docker inspect '+this.parameters+' |grep -m1 \\"IPAddress\\"|cut -d \"\\"\" -f 4').toString();
	    var resultat='invalide';
	    if(this.code!=null && this.code!=''){
		//Exécute le test
		try{
		    resultat=execSync('docker exec -t '+this.parameters+' bash -c "if [[ ' + this.code.replace(/([\\\/"$`])/g, '\\$1') + ' ]]; then echo valide; else echo invalide; fi"').toString();
		}
		catch(err){
		    resultat="Erreur";
	    ...
	    success(this.parameters, net_info[0]['NetworkSettings']['Ports']['4200/tcp'][0]['HostIp'], net_info[0]['NetworkSettings']['Ports']['4200/tcp'][0]['HostPort'], resultat);
	}
	else{
	    var st = 'bash -c "docker run --rm -p 4200 -h '+this.vm_name+' -d '+this.vm_name+'"'; // > '+sandbox.path + sandbox.folder + '/cid"';

	    //log the statement in console
	    console.log(st);

	    //execute the Docker
	    var contid=execSync(st).toString();
	    console.log("------------------------------")
	    console.log("Created : "+contid);

	    var net_info=JSON.parse(execSync('docker inspect '+contid));

	    success(contid, net_info[0]['NetworkSettings']['Ports']['4200/tcp'][0]['HostIp'], net_info[0]['NetworkSettings']['Ports']['4200/tcp'][0]['HostPort'], '');
